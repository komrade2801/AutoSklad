...
class Tools(Base, Model):
    ...
    plan_id = Column(Integer, ForeignKey('Plan.id'),
                     nullable=True, comment='Идентификатор плана')
    tool_type_id = Column(Integer, ForeignKey("ToolTypes.id"),
                          nullable=True, comment="Ключ на тип инструмента")
    ...
    groups_id = Column(Integer, ForeignKey('Group.id'),
                       nullable=True, comment='Внешний ключ на таблицу Group')
    ...
    def tool_type(self):
        if 'ToolTypes' not in Base.metadata.tables:
            ...
            ToolTypes = Base.metadata.tables['ToolTypes'].class_
        return relationship(ToolTypes, back_populates='Tools')
