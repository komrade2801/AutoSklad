...
LISTEN_PORT = 8765
PEER_URI = f"ws://192.168.0.10:{LISTEN_PORT}"
...
async def main():
    server = await websockets.serve(echo_server, "192.168.0.10", LISTEN_PORT)
    print(f"WebSocket-сервер запущен на ws://192.168.0.10:{LISTEN_PORT}")
    try:
        await ws_client()
    finally:
        server.close()
