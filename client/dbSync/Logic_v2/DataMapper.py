...
# #     #     result: Dict[str, Any] = {}
# #     #     print(
# #     #         f'[ПОТОК][{threading.current_thread().name}][DataMapper][map_outgoing] - table: {table}, record: {record}, base_map: {base_map}, converters: {converters}, type_map: {type_map}. Текущее время: {datetime.datetime.now()}')
# #     #     for local_field, value in record.items():
# #     #         remote_field = reverse_map.get(local_field)
# #     #         if not remote_field:
# #     #             continue
...
# #     #             except Exception as ex:
# #     #                 print(
# #     #                     f'[ПОТОК][{threading.current_thread().name}][DataMapper][map_outgoing][ERROR] - error: {ex}, подробности: - {traceback.format_exc()}. Текущее время: {datetime.datetime.now()}')
# #     #                 continue
# #     #         elif local_field in type_map:
# #     #             try:
# #     #                 value = type_map[local_field](value)
