...
#   ️ Список изменений
#   TransformRules TypedDict
#   Описывает структуру правил: incoming, validate, outgoing.
#   DiagnosticLogger
#   Бросил прямой logging в пользу DiagnosticLogger.
#   Параметр strict.
#   Включает жёсткую обработку: исключения при ошибках.
#   Динамическая регистрация
#   register_rule позволяет добавлять правила на лету.
#   Копирование данных
#   Безопасно копируем raw и mapped перед трансформацией, чтобы избежать побочных эффектов.
#   Единообразная структура
#   Методы preprocess, validate, postprocess читаются аналогично.
#   Докстринги с архитектурным контекстом
#   Расположение в цепочке SyncProcessor → DataMapper → DataTransformer.
#   Примеры Sequence Diagram
#   Встроен краткий протокол вызовов.
#    Дополнительная информация и советы
#   Coverage
#   Unit-тесты для всех этапов, включая негативные сценарии с strict=True.
#   Метрики
#   Отслеживать количество успешных/проваленных валидаций и обработок.
#   Асинхронность
#   Для тяжёлых правил можно запускать в фоне или через async def.
#   Производительность
#   При большом количестве пред/пост правил можно предусмотреть batch-преобразования.
#   Расширяемость
#   Можно добавить stage='both' для функций, применяемых в incoming и outgoing.
#   Интеграция
#   SyncProcessor легко внедрит DataTransformer перед/после DataMapper.
#