...
def test_write_db_mass_drop_tools_by_plan_success(mapper, fake_data):
    ...
    for i in range(2):
        ...
        if not status:
            index = max(mapper.e_status.get_all_ids()) + 1
            mapper.e_status.add(index=index, stype="mass_drop_init", description="Объявлена массовая выгрузка")
...
def test_write_db_mass_drop_tools_by_free_success(mapper, fake_data):
    """
    Проверяет успешное выполнение массового удаления инструментов без привязки к плану.
    """
    tools_data = []
    cells_data = []
    ...
    for i in range(2):
        ...
        mapper.e_tools.add_tool(
            ...
            description=description,
            img=img,
            plan_id=None,  # Без привязки к плану
            groups_id=None
        )
        tool = mapper.e_tools.get(tools_indx)
        tools_data.append(tool)
    ...
    action = 'write_db_mass_drop_tools_by_free'
    result = mapper.execute(action, tools_data, cells_data)

    assert result is True


def test_write_db_mass_drop_tools_by_free_length_mismatch(mapper, fake_data):
    """
    Проверяет ошибку при несовпадении количества инструментов и ячеек для свободного удаления.
    """
    tools_data = [
        Tools(
            id=1,
    ...
    with pytest.raises(MassDropLenEqCellToolsError):
        mapper.execute("write_db_mass_drop_tools_by_free", tools_data, cells_data)
