...
function updateToolsJSON(jsonObjectTools, plansIndex, groupIndex, toolIndex) {
    //console.log("updateToolsJSON успешно вызвана")

    // Получаем группу и инструмент по индексу
    const group = jsonObjectTools.plans[plansIndex].groups[groupIndex];
  ...
  droppableElements.forEach(droppable => {
    if (!droppable.dataset.initialized) {
      ...
      droppable.addEventListener("drop", (event) => {
        ...
        updateToolsJSON(jsonObjectTools, plansIndex, groupIndex, toolIndex);
        updateCellsJSON(jsonObjectCells, planName, toolName, cellId);
