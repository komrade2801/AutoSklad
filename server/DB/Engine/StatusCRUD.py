...
class EngineStatus(BaseCRUD):
    ...
    def add(self, index: int, stype: str, description: Optional[str] = None,
        ...
        try:
            return super().add(index=index, stype=stype, description=description, created_at=created_at)
    ...
    def get_status_by_stype(self, stype: str) -> Optional[Status]:
        """
        Получает статус по его типу (stype).

        :param stype: Тип статуса для поиска.
        :return: Объект Status, если найден, иначе None.
        """
        return self.session.query(Status).filter_by(stype=stype).first()
