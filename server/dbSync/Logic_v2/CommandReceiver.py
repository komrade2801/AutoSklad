...
import threading
from dbSync.Transport.TransportService import TransportService, logger
...
class CommandReceiver:
    ...
    def _load_last_synced(self) -> str:
        ...
        if not os.path.exists(path):
            ...
            print(f'[ПОТОК][{thread_name}][CommandReceiver][_load_last_synced] Файл не найден (первый запуск?) → создаём пустой. @ {datetime.now()}')
            try:
                with open(path, "w", encoding="utf-8") as f:
                    f.write("")  # создаём пустой файл
            except Exception as e:
                print(f'[ПОТОК][{thread_name}][CommandReceiver][_load_last_synced][ERROR] Не удалось создать файл: {e} @ {datetime.now()}')
            return ""
        ...
        try:
            ...
            print(f'[ПОТОК][{thread_name}][CommandReceiver][_load_last_synced][WARN] Неверный формат даты в файле → сбрасываем. @ {datetime.now()}')
            return ""
        except Exception as e:
            print(f'[ПОТОК][{thread_name}][CommandReceiver][_load_last_synced][ERROR] Неожиданная ошибка чтения: {e} @ {datetime.now()}')
            return ""
    ...
    def fetch_and_apply(self) -> None:
        ...
        try:
            ...
            print(f'[ПОТОК][{threading.current_thread().name}][CommandReceiver][fetch_and_apply][ERROR] - error: {e}. [{datetime.now()}]')
            # if self.logger:
            #     self.logger.log_error(f"Pull request failed: {e}, подробности: - {traceback.format_exc()}")
            return
        except RuntimeError as re:
            logger.error(f"[sync.{self.device_id}] Сервер синхронизации недоступен: {re}")
            return
        except ConnectionError as ce:
            logger.error(f"[sync.{self.device_id}] Не удалось подключиться к {self.endpoint}: {ce}")
            return  # или прерываем текущую итерацию, оставляем состояние без изменений
