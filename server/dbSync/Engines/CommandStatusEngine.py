...
class CommandStatusCRUD(BaseCRUD):
    ...
    def add_status(self, command_id: int, status: str) -> bool:
        ...
        try:
            print( f"[ПОТОК][{threading.current_thread().name}][CommandStatusEngine][add_status][INFO] - command_id: {command_id}, status: {status}. [{datetime.now()}]")
            return super().add(
            ...
            print(f"[ПОТОК][{threading.current_thread().name}][CommandStatusEngine][add_status][ERROR][IntegrityError] - error: {e}, подробности: - {traceback.format_exc()}. [{datetime.now()}]")
