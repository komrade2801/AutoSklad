...
- **`BatchProcessor`**: Executes local database changes in atomic transactions with ALL_OR_NOTHING rollback strategy
...
The client now implements a sophisticated **compensation-based rollback** system using the **Saga Pattern** to ensure data integrity during batch synchronization operations. This prevents partial failures from leaving the database in an inconsistent state.

#### SnapshotManager (`dbSync/Logic_v2/SnapshotManager.py`)

**Purpose**: Captures database record state BEFORE modifications to enable precise rollback through compensation.
...
1. **`capture_snapshot(command_id, table, record_id, operation)`**
   - Called BEFORE each UPDATE/DELETE operation
   - Queries current record state from `work.db`
   - Serializes state to JSON
   - Stores in `CommandSnapshot` table in `sync.db`
   - Returns `True` on success

2. **`generate_compensation_for_command(command_id)`**
   - Retrieves snapshot for failed command
   - Generates inverse operation:
     - INSERT → DELETE (remove created record)
     - UPDATE → UPDATE (restore old values)
     - DELETE → INSERT (restore deleted record)
   - Returns compensation operation dict or None

3. **`cleanup_old_snapshots(older_than_days=30)`**
   - APScheduler job (daily at 3:00 AM)
...
# Snapshot stored: {"id": 5, "status_id": 3, "tools_id": null, "device_id": 1}

# If batch fails, generate compensation:
compensation = snapshot_manager.generate_compensation_for_command(123)
# Returns: {"table": "Cell", "operation": "update", "id": 5, 
#           "data": {"status_id": 3, "tools_id": null}}
```
...
CREATE TABLE CommandSnapshot (
  id INTEGER PRIMARY KEY,
  command_id INTEGER,           -- FK to Command.id
  table_name VARCHAR,            -- "Cell", "Tools", etc.
  record_id INTEGER,             -- Record being modified
  snapshot_data TEXT,            -- JSON: {"field": "value", ...}
...
**Purpose**: Prevents duplicate batch execution using unique idempotency tokens.

**Key Responsibilities**:
- Generate unique tokens for each batch operation
- Check if batch has already been processed
- Track token status and expiration
- Automatic cleanup of expired tokens (24+ hours)

**Core Methods**:

1. **`generate_token(batch_id=None)`**
   - Generates UUID v4 token
   - Creates `IdempotencyToken` record
...
3. **`mark_completed(token, batch_id, status='COMPLETED')`**
   - Updates token status after batch processing
   - Links token to batch execution record
   - Stores completion timestamp

4. **`cleanup_expired_tokens()`**
   - APScheduler job (daily)
   - Removes tokens older than expiration
   - Returns count of deleted tokens

**Example Idempotency Flow**:
```python
# Client generates token before sending batch
token = idempotency_manager.generate_token()
# Returns: "a1b2c3d4-e5f6-7890-abcd-ef1234567890"

# Server checks for duplicates
if idempotency_manager.is_duplicate(token):
    return cached_result  # Don't re-execute

# Process batch...
results = batch_processor.execute_batch(commands)

# Mark as complete
idempotency_manager.mark_completed(token, batch_id, 'COMPLETED')
```
...
CREATE TABLE IdempotencyToken (
  id INTEGER PRIMARY KEY,
  token VARCHAR UNIQUE,          -- UUID token
  batch_id VARCHAR,              -- Associated batch
...
The `BatchProcessor` now implements **ALL_OR_NOTHING** rollback strategy:

**Key Enhancements**:
1. **Savepoint-Based Transactions**: Uses `begin_nested()` for atomic batch execution
2. **Pre-Execution Snapshots**: Captures state before each command
3. **Automatic Rollback**: Single failure triggers complete batch rollback
4. **Result Tracking**: Detailed per-command success/failure reporting

**Execution Flow**:
```python
with work_db.begin_nested():  # SAVEPOINT
    ...
    # ROLLBACK SAVEPOINT
    # All changes undone, snapshots preserved
    return results  # Contains partial results up to failure
```

**Rollback Behavior**:
- **Success**: All commands complete → COMMIT transaction
- **Failure**: Any error → ROLLBACK entire batch
- **State**: Database returns to pre-batch state
- **Audit**: Snapshots remain in `sync.db` for debugging
...
- **Rollback Protection**: Snapshots in `sync.db` survive `work.db` rollbacks
...
- **Failure Recovery**: Batch failures don't affect local hardware state
